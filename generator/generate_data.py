...
def post_process_gpt3_response(num_prompt_instructions, response, discarded_file):
    ...
    for inst in raw_instructions:
        ...
        # filter those starting with non-English character and not a Korean character
        if not inst[0].isascii() and not is_korean_char(inst[0]):
            writeline2file(
                discarded_file,
                "Discarded instruction(began with non-ascii and non-Korean): " + repr(splitted_data),
            )
            discarded += 1
            continue
        instructions.append(
